...
async function run(): Promise<void> {
  try {
    ...
    const pull_request_description: string =
      context.payload.pull_request?.body ?? '';
    const ab_lookup_match: RegExpMatchArray | null =
      pull_request_description.match(/AB#(\d+)/g);
    ...
    if (context.eventName === 'pull_request') {
      last_comment_posted = await getLastComment(
        octokit,
        repository_owner,
        repository_name,
        pull_request_number,
        ...
        if (
          ...
          if (
            ...
            if (
              ...
              console.log(
                `Deleting last comment posted by action: ${last_comment_posted.id}`,
              );
          ...
          if (
            ...
            core.setFailed(
              `Description contains AB#${work_item_id} but the Bot could not link it to an Azure Boards work item`,
            );
            return;
          }

          core.warning(
            `Description contains AB#${work_item_id} and waiting for the azure-boards[bot] to validate the link`,
          );
        }
